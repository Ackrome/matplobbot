...
### 🟠 SEC-5: Утечка внутренней топологии сети и домашних сервисов в `Caddyfile`
- **Приоритет:** MEDIUM (P2)
...
### 🟠 BUG-5: Потеря состояния инлайн-кнопок при перезапуске бота (`LRUCache`)
- **Приоритет:** MEDIUM (P2)
...
- **Рекомендация:** Сохранять маппинг хэшей путей в Redis с TTL (например, 7-14 дней), чтобы состояние переживало рестарты бота.
...
### 🧩 PROD-2: Диссонанс функционала расписания: Web vs Telegram Bot
- **Проблема:**
...
### 🧩 PROD-4: Забытая страница деталей пользователя (`/users/{user_id}`)
...
### 🧩 PROD-6: Привязка конспектов GitHub только к ветке `main`
...
### 🧩 PROD-7: Ограничение поиска "Семантический поиск"
...
### ⚠️ UX-4: Разрозненное и несогласованное версионирование статики
