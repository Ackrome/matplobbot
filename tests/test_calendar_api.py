...
try:
    ...
    with patch.dict(
        ...
        from fastapi_stats_app.routers import calendar_router_v2 as calendar_router
...
class TestCalendarAPI(unittest.TestCase):
    ...
    def test_public_feed_passes_source_updated_at_to_ical_generator(self):
        active_subscriptions = [
            {
                "id": 99,
                "is_active": True,
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            captured["schedule"][0]["source_updated_at"],
