...
class TextSearchEngine:
        ...
        async with get_session() as session:
            stmt = (
                ...
                .on_conflict_do_update(
                    ...
                    set_=dict(content=content, metadata=metadata, created_at=func.now()),
                )
            )
    ...
    async def search(self, query: str, source_type: str = None, top_k: int = 10) -> list[dict]:
        ...
        stmt = select(
            SearchDocument.source_path, SearchDocument.content, SearchDocument.metadata_, rank_col
        ...
        async with get_session() as session:
            if source_type:
                await session.execute(
                    delete(SearchDocument).where(SearchDocument.source_type == source_type)
                )
            else:
