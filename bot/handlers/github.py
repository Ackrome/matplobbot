...
class GitHubManager:
    ...
    def _register_handlers(self):
        ...
        self.router.callback_query(F.data.startswith("lec_browse_repo:"))(
            self.cq_lec_browse_repo_selected
        )
        ...
        self.router.callback_query(
            RepoManagement.choose_repo_for_search, F.data.startswith("lec_search_repo:")
        )(self.cq_lec_search_repo_selected)
        self.router.message(MarkdownSearch.query)(self.process_md_search_query)
        self.router.callback_query(F.data.startswith("md_search_page:"))(
            self.cq_md_search_pagination
        )
    ...
    async def lec_all_command(self, message: Message, state: FSMContext):
        ...
        for repo in repos:
            ...
            builder.row(
                InlineKeyboardButton(text=repo, callback_data=f"lec_browse_repo:{repo_hash}")
            )

        await message.answer(
            translator.gettext(lang, "github_choose_repo_browse"), reply_markup=builder.as_markup()
        )
    ...
    async def cq_lec_browse_repo_selected(self, callback: CallbackQuery):
        ...
        await github_display.display_lec_all_path(
            callback.message, repo_path=repo_path, path="", is_edit=True, user_id=user_id
        )
    ...
    async def cq_lec_all_show_file(self, callback: CallbackQuery):
        ...
        if not file_path:
            await callback.answer(
                translator.gettext(lang, "github_file_info_outdated"), show_alert=True
            )
            return
    ...
    async def _get_md_search_results_keyboard(
        ...
        for item in page_items:
            ...
            builder.row(
                InlineKeyboardButton(
                    text=f"📄 {item['path']}", callback_data=f"show_md_hash:{path_hash}"
                )
            )
            ...
            if page > 0:
                pagination_buttons.append(
                    InlineKeyboardButton(
                        ...
                        callback_data=f"md_search_page:{page - 1}",
                    )
                )
            pagination_buttons.append(
    ...
    async def lec_search_command(self, message: Message, state: FSMContext):
        ...
        if len(repos) == 1:
            ...
            await message.answer(
                translator.gettext(lang, "github_search_prompt", repo_path=repos[0]),
                parse_mode="markdown",
        ...
        for repo in repos:
            ...
            kb.code_path_cache[repo_hash] = repo
            builder.row(
                InlineKeyboardButton(text=repo, callback_data=f"lec_search_repo:{repo_hash}")
            )

        await state.set_state(RepoManagement.choose_repo_for_search)
        await message.answer(
            translator.gettext(lang, "github_choose_repo_search"), reply_markup=builder.as_markup()
        )
    ...
    async def cq_lec_search_repo_selected(self, callback: CallbackQuery, state: FSMContext):
        ...
        await callback.message.edit_text(
            translator.gettext(lang, "github_search_prompt", repo_path=repo_path),
            parse_mode="markdown",
        )
        await callback.answer()

    async def _handle_md_search_success(
        self, status_msg: Message, user_id: int, query: str, repo_to_search: str, results: list
    ):
        lang = await translator.get_language(user_id)
        await redis_client.set_user_cache(
            user_id, "md_search", {"query": query, "results": results, "repo_path": repo_to_search}
        )
    ...
    async def process_md_search_query(self, message: Message, state: FSMContext):
        ...
        status_msg = await message.answer(
            f"🔍 Ищу '{query}' в `{repo_to_search}`...", parse_mode="Markdown"
        )
    ...
    async def cq_md_search_pagination(self, callback: CallbackQuery):
        ...
        if not search_data:
            await callback.answer(
                translator.gettext(lang, "search_results_outdated"), show_alert=True
            )
        ...
        results, query, repo_path = (
            search_data["results"],
            search_data["query"],
            search_data["repo_path"],
        )
        total_pages = (len(results) + SEARCH_RESULTS_PER_PAGE - 1) // SEARCH_RESULTS_PER_PAGE
    ...
    async def _show_repo_management_menu(
        self, message: Message, user_id: int, state: FSMContext, is_edit: bool = False
    ):
    ...
    async def cq_manage_repos(self, callback: CallbackQuery, state: FSMContext):
        ...
        await self._show_repo_management_menu(
            callback.message, callback.from_user.id, state, is_edit=True
        )
    ...
    async def cq_add_new_repo_prompt(self, callback: CallbackQuery, state: FSMContext):
        ...
        await callback.message.edit_text(
            translator.gettext(lang, "repo_add_new_prompt"), reply_markup=None
        )
        await callback.answer()
    ...
    async def process_add_repo(self, message: Message, state: FSMContext):
        ...
        if re.match(r"^[a-zA-Z0-9_-]+/[a-zA-Z0-9_.-]+$", repo_path):
            ...
            await message.answer(
                translator.gettext(
                    lang,
                    "repo_add_success" if success else "repo_add_already_exists",
                    repo_path=repo_path,
                ),
                parse_mode="markdown",
            )
    ...
    async def cq_delete_repo(self, callback: CallbackQuery, state: FSMContext):
        ...
        await callback.answer(
            translator.gettext(lang, "repo_deleted", repo_path=repo_path), show_alert=False
        )
    ...
    async def cq_edit_repo_prompt(self, callback: CallbackQuery, state: FSMContext):
        ...
        await callback.message.edit_text(
            translator.gettext(lang, "repo_edit_prompt", old_repo_path=repo_path),
            parse_mode="markdown",
        )
        await callback.answer()
    ...
    async def _run_indexing_task(self, message: Message, repo_path: str):
        status_msg = await message.answer(
            f"⏳ Индексация `{repo_path}` началась...", parse_mode="Markdown"
        )
        try:
            await index_github_repository(repo_path)
            await status_msg.edit_text(
                f"✅ Индексация `{repo_path}` завершена! Теперь поиск работает по смыслу.",
                parse_mode="Markdown",
            )
        except Exception as e:
