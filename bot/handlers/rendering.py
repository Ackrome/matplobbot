...
class RenderingManager:
    ...
    def _register_handlers(self):
        self.router.message(Command("latex"))(self.latex_command)
        self.router.message(LatexRender.formula)(self.process_latex_formula)
        self.router.message(Command("mermaid"))(self.mermaid_command)
    ...
    async def latex_command(self, message: Message, state: FSMContext):
        lang = await translator.get_language(message.from_user.id)  # <<< ДОБАВЛЕНО
        if message.chat.type != "private":
            await message.reply(translator.gettext(lang, "rendering_private_only"))
            return

        lang = await translator.get_language(message.from_user.id)
        await state.set_state(LatexRender.formula)
        await message.answer(
            translator.gettext(lang, "latex_prompt"), reply_markup=ReplyKeyboardRemove()
        )
    ...
    async def process_latex_formula(self, message: Message, state: FSMContext):
        ...
        try:
            ...
            logging.error(
                f"Непредвиденная ошибка при рендеринге LaTeX для '{formula}': {e}", exc_info=True
            )
            await status_msg.edit_text(translator.gettext(lang, "latex_unexpected_error", error=e))
    ...
    async def mermaid_command(self, message: Message, state: FSMContext):
        ...
        await message.answer(
            translator.gettext(lang, "mermaid_prompt"), reply_markup=ReplyKeyboardRemove()
        )

    async def process_mermaid_code(self, message: Message, state: FSMContext):
