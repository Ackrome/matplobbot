...
class ScheduleManager:
    ...
    def _register_handlers(self):
        ...
        self.router.callback_query(F.data.startswith("sch_open_calendar:"))(
            self.handle_open_calendar
        )
        ...
        self.router.callback_query(F.data.startswith("sch_subscribe_hash:"))(
            self.handle_subscribe_button
        )
        self.router.message(ScheduleStates.awaiting_subscription_time)(
            self.handle_subscription_time
        )
        ...
        self.router.callback_query(F.data == "mysch_filters:main")(
            self.handle_myschedule_filters_menu
        )
        ...
        self.router.callback_query(F.data.startswith("mysch_ical:"))(
            self.handle_myschedule_export_ical
        )
    ...
    async def cmd_schedule(self, message: Message, state: FSMContext):
        ...
        await message.answer(translator.gettext(lang, "schedule_welcome"), reply_markup=keyboard)
    ...
    async def _perform_search_and_reply(
        self, message: Message, status_msg: Message, query: str, search_type: str
    ):
    ...
    async def handle_result_selection(self, callback: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            entity_name = await self._resolve_entity_name(
                user_id, entity_type, entity_id, schedule_data
            )
            ...
            formatted_text = await format_schedule(
                schedule_data, lang, entity_name, entity_type, user_id, start_date=today.date()
            )
            ...
            await self._send_actions_menu(
                ...
                entity_id,
                entity_name,
                view_type="daily_initial",
            )
        except Exception as e:
            logging.error(
                f"Failed to get today's schedule for {entity_type}:{entity_id}. Error: {e}",
                exc_info=True,
            )
    ...
    async def handle_open_calendar(self, callback: CallbackQuery):
        ...
        calendar_keyboard = build_calendar_keyboard(
            now.year, now.month, entity_type, entity_id_for_callback, lang
        )
        await callback.message.edit_text(
            translator.gettext(lang, "schedule_select_date"), reply_markup=calendar_keyboard
        )
    ...
    async def handle_calendar_navigation(self, callback: CallbackQuery):
        ...
        try:
            ...
            if action == "prev_month":
                ...
                month += 1
                year = year + 1 if month == 13 else year
                ...
                if now.year == int(year_str) and now.month == int(month_str):
                    await callback.answer(
                        translator.gettext(
                            await translator.get_language(
                                callback.from_user.id, callback.message.chat.id
                            ),
                            "calendar_already_on_today",
                        ),
                        show_alert=False,
                    )
                    return
            ...
            try:
                await callback.message.edit_reply_markup(
                    reply_markup=build_calendar_keyboard(year, month, entity_type, entity_id, lang)
                )
            except TelegramBadRequest as e:
                if "message is not modified" in e.message:
                    await callback.answer(
                        translator.gettext(lang, "calendar_already_on_today"), show_alert=False
                    )
                else:
                    raise
    ...
    async def handle_back_to_calendar(self, callback: CallbackQuery):
        ...
        try:
            ...
            lang = await translator.get_language(callback.from_user.id, callback.message.chat.id)
            calendar_keyboard = build_calendar_keyboard(
                year, month, entity_type, entity_id, lang, selected_date=selected_date
            )
            await callback.message.edit_text(
                translator.gettext(lang, "schedule_select_date"), reply_markup=calendar_keyboard
            )
    ...
    async def _send_actions_menu(
        ...
        await message.answer(
            translator.gettext(lang, "schedule_actions_prompt"), reply_markup=keyboard.as_markup()
        )

    def _build_schedule_actions_keyboard(
        self,
        lang: str,
        entity_type: str,
        entity_id: str,
        ...
        subscribe_button_data = build_search_results_keyboard(
            [
                {
                    "label": translator.gettext(lang, "schedule_subscribe_button"),
                    "id": f"{entity_type}:{entity_id}:{entity_name}",
                }
            ],
            "subscribe",
        )
        ...
        if view_type == "daily_initial":
            ...
            builder.row(
                InlineKeyboardButton(
                    text=translator.gettext(lang, "schedule_export_ical"),
                    callback_data=ical_callback,
                )
            )
            builder.row(
                InlineKeyboardButton(
                    text=translator.gettext(lang, "schedule_view_calendar"),
                    callback_data=open_calendar_callback,
                )
            )
            ...
            ical_callback = (
                f"sch_export_ical:{entity_type}:{safe_entity_id}:{date_info['date_str']}"
            )
            builder.row(
                InlineKeyboardButton(
                    text=translator.gettext(lang, "schedule_export_ical"),
            ...
            builder.row(
                InlineKeyboardButton(
                    text=translator.gettext(lang, "schedule_back_to_calendar"),
                    callback_data=back_to_cal_callback,
                )
            )
        ...
        builder.row(
            InlineKeyboardButton(
                text=translator.gettext(lang, "schedule_back_to_results"),
                callback_data="sch_back_to_results",
            )
        )
        return builder
    ...
    async def handle_date_selection(self, callback: CallbackQuery):
        ...
        try:
            ...
            logging.error(
                f"Failed to get schedule for {entity_type}:{entity_id}. Error: {e}", exc_info=True
            )
            await callback.message.edit_text(translator.gettext(lang, "schedule_api_error"))
    ...
    async def handle_week_selection(self, callback: CallbackQuery):
        ...
        try:
            ...
            await self._send_actions_menu(
                callback.message, lang, entity_type, entity_id, entity_name, "weekly", date_info
            )
        except Exception as e:
            logging.error(
                f"Failed to get weekly schedule for {entity_type}:{entity_id}. Error: {e}",
                exc_info=True,
            )
            await callback.message.edit_text(translator.gettext(lang, "schedule_api_error"))

    async def _prepare_ical_file(
        self, user_id: int, entity_type: str, entity_id: str, start_date_str: str
    ) -> tuple[bytes, str, date, date]:
    ...
    async def handle_ical_export(self, callback: CallbackQuery):
        ...
        try:
            file_bytes, filename, start_date, end_date = await self._prepare_ical_file(
                user_id, entity_type, entity_id, start_date_str
            )
            await callback.message.answer_document(
                document=BufferedInputFile(file_bytes, filename=filename),
                caption=translator.gettext(
                    lang,
                    "schedule_export_ical_caption",
            ...
            logging.error(
                f"Failed to generate iCal for {entity_type}:{entity_id}. Error: {e}", exc_info=True
            )
    ...
    async def handle_subscribe_button(self, callback: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            await callback.answer(
                translator.gettext(lang, "schedule_subscribe_error"), show_alert=True
            )
            return
        await state.set_state(ScheduleStates.awaiting_subscription_time)
        await state.update_data(
            sub_entity_type=entity_type, sub_entity_id=entity_id, sub_entity_name=entity_name
        )
    ...
    async def handle_subscription_time(self, message: Message, state: FSMContext):
        ...
        try:
            ...
            sub_id = await database.add_schedule_subscription(
                ...
                sub_data["sub_entity_name"],
                notification_time,
            )

            sem_start, sem_end = get_semester_bounds()
            logging.info(
                f"Fetching full semester schedule for subscription: {sub_data['sub_entity_name']} ({sem_start} - {sem_end})"
            )
            ...
            new_hash = hashlib.sha256(
                json.dumps(schedule_data_for_hash, sort_keys=True).encode()
            ).hexdigest()
            await database.update_subscription_hash(sub_id, new_hash)

            await redis_client.set_user_cache(
                user_id, f"schedule_data:{sub_id}", json.dumps(schedule_data_for_hash), ttl=None
            )

            webcal_btn = InlineKeyboardButton(
                text=translator.gettext(lang, "kb_cal_webcal_button"),
                callback_data="mysch_cal_link",
            )
            ...
            if sub_data["sub_entity_type"] == "group":
                ...
                if unique_modules:
                    ...
                    base_keyboard = await get_modules_keyboard(
                        unique_modules, current_selected, sub_id
                    )
    ...
    async def cmd_my_schedule(self, message: Message, state: FSMContext):
        ...
        builder.row(
            InlineKeyboardButton(
                ...
                callback_data=f"mysch_ical:{today_str}:1",
            )
        )
        builder.row(
            InlineKeyboardButton(
                text=translator.gettext(lang, "myschedule_full_calendar_button"),
                callback_data="mysch_open_cal",
            )
        )
        builder.row(
            InlineKeyboardButton(
        ...
        await message.answer(
            translator.gettext(lang, "myschedule_actions_header"), reply_markup=builder.as_markup()
        )
    ...
    async def _render_calendar(self, callback: CallbackQuery, year: int, month: int):
        ...
        await redis_client.set_user_cache(
            user_id, "mysch_current_view", {"year": year, "month": month}
        )
    ...
    async def handle_myschedule_day(self, callback: CallbackQuery):
        ...
        schedule = await get_aggregated_schedule(
            user_id, active_subs, target_date, target_date, filters
        )
    ...
    async def handle_myschedule_export_ical(self, callback: CallbackQuery):
        ...
        await callback.message.answer_document(
            document=BufferedInputFile(ical_data, filename=filename), caption=caption
        )
    ...
    async def _resolve_entity_name(
        ...
        if cached_search and cached_search.get("results"):
            item = next(
                (i for i in cached_search["results"] if str(i["id"]) == str(entity_id)), None
            )
            if item:
                return item["label"]
        ...
        for item_json in history_raw:
            try:
                ...
                if (
                    str(item.get("entity_id")) == str(entity_id)
        ...
        sub = next(
            (
                s
                for s in subs
                if str(s["entity_id"]) == str(entity_id) and s["entity_type"] == entity_type
            ),
            None,
        )
        if sub:
