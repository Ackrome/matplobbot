...
class MentionedFilter(Filter):
    ...
    async def __call__(self, message: Message, bot: Bot) -> bool:
        ...
        return any(
            entity.type == "mention"
            and message.text[entity.offset : entity.offset + entity.length] == f"@{bot_username}"
...
class BaseManager:
    ...
    def _register_handlers(self):
        ...
        self.router.callback_query(F.data == "go_to_onboarding_rendering")(
            self.onboarding_rendering
        )
        ...
        # Group chat handlers
        self.router.message(CommandStart(), F.chat.type.in_({"group", "supergroup"}))(
            self.command_start_group
        )
        self.router.message(Command("help"), F.chat.type.in_({"group", "supergroup"}))(
            self.command_start_group
        )  # Alias /help to /start in groups
    ...
    async def onboarding_welcome(self, event: Message | CallbackQuery, state: FSMContext):
        ...
        builder.row(
            InlineKeyboardButton(
                text=translator.gettext(lang, "onboarding_btn_next"),
                callback_data="start_onboarding_tour",
            )
        )
        builder.row(
            InlineKeyboardButton(
                text=translator.gettext(lang, "onboarding_btn_skip"),
        ...
        if isinstance(event, CallbackQuery):
            await message.edit_text(
                translator.gettext(lang, "start_onboarding_1"), reply_markup=builder.as_markup()
            )
        else:
    ...
    async def onboarding_language_choice(self, message: Message, state: FSMContext):
        ...
        await message.answer(
            translator.gettext("ru", "start_choose_language"), reply_markup=builder.as_markup()
        )
    ...
    async def onboarding_github(self, callback: CallbackQuery, state: FSMContext):
        ...
        builder.row(
            InlineKeyboardButton(
                text=translator.gettext(lang, "onboarding_btn_add_repo"),
        ...
        await callback.message.edit_text(
            translator.gettext(lang, "start_onboarding_2"), reply_markup=builder.as_markup()
        )
    ...
    async def onboarding_library(self, callback: CallbackQuery, state: FSMContext):
        ...
        builder.row(
            InlineKeyboardButton(
                text=translator.gettext(lang, "onboarding_btn_browse_library"),
        ...
        builder.row(
            InlineKeyboardButton(
                text=translator.gettext(lang, "onboarding_btn_next"),
                callback_data="go_to_onboarding_rendering",
            )
        )
        await callback.message.edit_text(
            translator.gettext(lang, "start_onboarding_3"), reply_markup=builder.as_markup()
        )
        await callback.answer()
    ...
    async def onboarding_rendering(self, callback: CallbackQuery, state: FSMContext):
        ...
        builder.add(
            InlineKeyboardButton(
                text=translator.gettext(lang, "onboarding_btn_try_latex"),
                callback_data="help_cmd_latex",
            )
        )
        builder.add(
            InlineKeyboardButton(
                text=translator.gettext(lang, "onboarding_btn_next"),
                callback_data="go_to_onboarding_schedule",
            )
        )
        await callback.message.edit_text(
            translator.gettext(lang, "start_onboarding_4"), reply_markup=builder.as_markup()
        )
        await callback.answer()
    ...
    async def onboarding_schedule(self, callback: CallbackQuery, state: FSMContext):
        ...
        builder.add(
            InlineKeyboardButton(
                text=translator.gettext(lang, "onboarding_btn_try_schedule"),
        ...
        builder.add(
            InlineKeyboardButton(
                text=translator.gettext(lang, "onboarding_btn_next"),
                callback_data="go_to_onboarding_final",
            )
        )
        await callback.message.edit_text(
            translator.gettext(lang, "start_onboarding_schedule"), reply_markup=builder.as_markup()
        )
        await callback.answer()
    ...
    async def onboarding_final(self, callback: CallbackQuery, state: FSMContext):
        ...
        builder = InlineKeyboardBuilder().row(
            InlineKeyboardButton(
                text=translator.gettext(lang, "onboarding_btn_finish"),
                callback_data="onboarding_finish",
            )
        )
        await callback.message.edit_text(
            translator.gettext(lang, "start_onboarding_5"), reply_markup=builder.as_markup()
        )
        await callback.answer()
    ...
    async def command_cycle_language_reply(self, message: Message):
        ...
        await message.answer(
            translator.gettext(new_lang, "settings_language_updated", lang_name=lang_name),
            reply_markup=await kb.get_main_reply_keyboard(user_id),
        )
    ...
    async def command_start_group(self, message: Message):
        ...
        keyboard.add(
            InlineKeyboardButton(
                text=translator.gettext(lang, "group_chat_start_button"),
                url=f"https://t.me/{bot_username}?start=start",
            )
        )
        await message.reply(
            translator.gettext(lang, "group_chat_start_text"), reply_markup=keyboard.as_markup()
        )
