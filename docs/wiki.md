...
- The proxy cleaner now exposes internal diagnostics endpoints on port `8080`: `/health`, `/diagnostics`, and `/recheck?group=telegram|openai|all`.
...
17. Use `http://proxy:8080/recheck?group=telegram` to force an immediate Telegram-side health recheck when troubleshooting provider failover.
