...
| --- | ---: | ---: | ---: | ---: | ---: |
| Bugs & Runtime Failures | 0 | 6 | 3 | 0 | 9 |
...
| **Итого активных** | **0** | **6** | **10** | **10** | **26** |

В первоначальном варианте было описано 33, а не 39 уникальных задач. Шесть пунктов исключены
из активного backlog после проверки, два пункта Sprint 1 выполнены, а оставшийся CSP-hardening
выделен из SEC-02 в отдельный SEC-04. Причины и результаты приведены в разделе 7.

## 3. P0 — активных задач нет

SEC-01 закрыт в Sprint 1 и перенесён в журнал выполненных пунктов.

## 4. P1 — безопасность и надёжность ключевых функций
...
| Сводка «39 задач» | Исправлена | В документе фактически было 33 уникальных ID. После исключения шести и выполнения двух осталось 25 исходных активных задач; отдельный CSP follow-up SEC-04 доводит текущий backlog до 26. |
...
1. BUG-08: корректная удалённая генерация `.env` в Jenkins.
2. BUG-04: outbox/retry для уведомлений об изменениях.
3. BUG-05: cached fallback ежедневной рассылки.
4. BUG-07: единый UTC-aware timestamp contract.
5. BUG-02: WebSocket proxy и runtime URL.
6. BUG-01: Unicode-safe ZIP filename.
